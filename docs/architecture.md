...
| `primer` (FP-018.1) | Deck primers: the Quill-Delta/plain-text provenance branch (`parse_primer` — card-link embeds are the only trusted card names; non-Delta JSON is refused), `<deckstem>.primer.md` sidecar read/write with the `primer-source` identity header (source id + description hash — `store_primer_sidecar` reports written/refreshed/unchanged/refused, `sidecar_identity_warning` names a mismatch; R3 F-07/F-08), the marked prompt clip (`clip_for_prompt`), word-bounded heading-aware win-line quoting | Deciding when to write a sidecar (`moxfield_import` does, on BOTH lanes since R3 F-06) or what the primer means (`adopt` / the judge consume). |
...
| `deck_judge` / `_deck_judge_prompt` | FP-016 Phase 1: blinded 6-judgment panel (3 per order, deck-keyed agreement, 5-of-panel supermajority), diff-focused intent-anchored prompts (free text — the deck's primer sidecar and `--preferences` — quoted inside a hash-keyed `<<<FREE-TEXT` fence the system prompt declares data, R3 F-05), schema-v4 `judge_verdict`/`judge_report` beside the sim verdict, `prompt_version` stamped on every report and tallied by `judge_agreement` | Advancing decks or gating anything — observe-only by contract. Game outcomes: Forge decides which deck is BETTER, only Forge. |
...
| `adopt` (FP-018.3) | `commander adopt`: deterministic/offline primer-vs-list explanation (card-link cross-check through `collection.match_key` — one front-face key, so DFC embeds match; resolved drift vs unrecognized links reported separately; role/theme packages; quoted win lines) + preference-steered (negation-aware, R3 F-04) like-for-like suggestions via `deck_builder_personalize.lift_swaps`, budget hard-clamped to the polish tier with auto-Protected primer-linked cards; unresolved cards are never proposed as cuts and a cold cache refuses the pass | Applying changes (read-only; suggestions only) and any overhaul — the rebuild tier is structurally unreachable here; `commander improve` owns big changes. |
...
| `web/_helpers.py` | Flask-independent helpers (`_apply_swaps_to_dck`, `_normalize_pasted_deck`, `_format_added_line`, etc.); `_BASIC_LANDS` constant; re-exports `atomic_io.atomic_write_text` (moved to core on 2026-09-03, R3 W-09) for the deck-writing blueprints | Route-specific logic. Each blueprint uses as needed. |
| `atomic_io` | `atomic_write_text` — temp file in the same dir + fsync + `os.replace`, bytes written verbatim (`newline=""`), mode inherited or explicit; every `.dck` and `config.json` writer routes through it (R3 W-08/W-09/W-10) | Deciding what to write. |
| `dck_utils` (readers) | `strip_bom` (the three `[metadata]` parsers call it) and `read_deck_text` — the ONE `.dck` reader: BOM-stripped UTF-8, non-UTF-8 decoded with replacement plus a loud per-file stderr warning (R3 W-03/W-04) | Choosing which files to read; callers glob. |
